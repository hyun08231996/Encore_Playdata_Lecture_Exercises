...
/*
 * @Controller
 *  --> @ReqeustMapping() 실행결과로 결과페이지 정보가 반환된다.
 *  
 * @RestController
...
public class SimpleController {
	...
	//객체가 리턴되는 경우...JSON으로 변환해서 리턴하기 떄문에 라이브러리를 반드시 추가해야 된다.
	@GetMapping("/custom")
	public Customer detail() {
		//비지니스로직 수행한 결과로 Customer 객체가 리턴...가정하자
	...
	public Map<Integer, Customer> sendCustom(){
		...
		map.put(1, new Customer("아이유1", "새벽길을 걸어요"));
		map.put(2, new Customer("아이유2", "새벽길을 걸어요"));
		map.put(3, new Customer("아이유3", "새벽길을 걸어요"));
		return map;
	}
}
