...
public class ProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		HttpSession session = request.getSession();//새롭게 만들어진 세션이 아니라 기존 세션을 계속 사용한다...로그아웃 해서 세션을 죽이지 않았다
		if(session.getAttribute("vo")!=null) {//로그인 되었다면...
			MemberVO vo = (MemberVO)session.getAttribute("vo");
			out.println("JSESSIONID :: "+session.getId()+"<br>"); //앞페이지에서 출력된 값과 동일한 값이 출력...같은 세션을 계속 사용
			out.println("VO :: "+vo);
