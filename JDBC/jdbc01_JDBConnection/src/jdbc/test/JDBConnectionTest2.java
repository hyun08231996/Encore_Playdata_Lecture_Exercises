...
/*
 * DB 서버 정보에 해당하는 것들을 상수값으로 세팅
 * 1. 드라이버 FQCN ---- 이 부분이 먼저 메모리에 로딩된 다음에 나머지 일들이 일어나야 한다.
 * 2. 서버주소, 계정이름, 비번...
 */
public class JDBConnectionTest2 {
	...
	public JDBConnectionTest2() {
		//jdbc 기본작업 진행...2,3,4단계 작업을 진행한다.
		try {
			//2.
...
//			int row = ps.executeUpdate();//업데이트된(추가) row의 갯수가 반환
